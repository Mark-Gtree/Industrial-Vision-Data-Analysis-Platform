...
bool ExcelOperator::open(QString path)
{
    ...
    try {
        m_pExcel->setControl("Excel.Application");//建立excel操作对象,并连接Excel控件
        m_pExcel->dynamicCall("SetVisible(bool)", true); //true 表示操作文件时可见，false表示为不可见
        m_pExcel->setProperty("DisplayAlerts", false);// 不显示任何警告信息， 如关闭时的是否保存提示
        pWorkbooks = m_pExcel->querySubObject("WorkBooks");// 获取工作簿(excel文件)集合
        pWorkbooks->dynamicCall("Add");//新建一个工作簿
        m_pWorkbook = m_pExcel->querySubObject("ActiveWorkBook");
        //qDebug()<<"excel path: "<<m_strPath;

        // 获取打开的excel文件中所有的工作sheet
        m_pWorksheets = m_pWorkbook->querySubObject("WorkSheets");// 获取当前工作簿
    } catch (...) {
        qCritical()<<"打开文件失败...";
        return false;
    }
...
bool ExcelOperator::close(QString path)
{
    m_endPath=path;
    qDebug()<<"excel close...";
    if (m_pExcel)
    {
        m_pWorkbook->dynamicCall("SaveAs(const QString&)", QDir::toNativeSeparators(m_endPath));
